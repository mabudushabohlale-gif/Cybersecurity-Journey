# Cybersecurity-Journey

## 👤About Me
I am a 15 year old aspiring Information Technology(IT) student. I created this repository to document my technical growth, lab notes and the concepts i master as i work towards a career in cybersecurity.

## 🎯My Goals
Foundation: Master the fundementals of Networking, Linux, and the OSI model.

Certification: Study for and eventually achieve entry-level certifications like CompTIA Security+, CompTIA Network+, eJPT, and SAL1

Practical skills: Move from theoery to hands on practice by completing labs on platforms like TryHackMe.

Specialization: Explore both Deffensive and Offensive Security to decide on a career path in Penetration testing or Security analyst.

## 📈Why i am Tracking my Progress on GitHub
Accountability: Publicly documenting my journey keeps me motivated and consistent.

Proof of Work: To build a living portfolio that demonstrates my technical writitng skills and hands on experiance to future mentors and employers.

Knowledge Base: To create a searchable personal library of commands, configurations, and walkthroughs tat i can refer back to at any time.

## 📚KnowledgeBase and Lab Notes
In this repository, you will find the modules and labs that i am currently working through.
